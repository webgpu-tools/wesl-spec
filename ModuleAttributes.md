...
We need a way of tagging modules as being permissible for wildcard imports.

In the future, adding module level metadata enables better tools.
For example, one can imagine marking a module as deprecated or as containing test code.

### Example

Wildcards in libraries are a semver hazard. More concretely, adding a name to a wildcardable module can break importers. Thus, opting into them is done explicitly. A module level attribute is used.
