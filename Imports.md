...
Furthermore, we want a module system that enables **importing shaders from libraries**. Authors should be able to upload WGSL shaders to existing package managers, which other users can consume.

Finally, we want to enable **tooling** which natively understands import statements. Such tooling should exist for the entire WebGPU ecosystem and not be tied to a specific toolchain.
...
import with the `@!wildcardable;` module attribute (see [Grammar](ModuleAttributes.md#grammar)):
